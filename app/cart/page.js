          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_360px] lg:items-start">
            <section className="space-y-4">
              {cart.map((item) => {
                ...
                return (
                  <article key={item._id} className="flex gap-4 rounded-3xl bg-white p-4 shadow-sm ring-1 ring-slate-200 sm:gap-6 sm:p-5">
                    <Link href={`/products/${item._id}`} className="shrink-0"><img src={item.image} alt={item.title} className="h-28 w-28 rounded-2xl object-cover sm:h-36 sm:w-36" /></Link>
